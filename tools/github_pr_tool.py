...
class GitHubPRStateMachine:
    ...
    #     """Retrieves GitHub token from Secret Manager with fallback to local GITHUB_TOKEN."""
    #     # 1. Try Secret Manager if configurations exist
    ...
    #             print(" > Retrieved GitHub Token from GCP Secret Manager.")
    #             return token_val
    #         except Exception as e:
    #             print(f" ⚠️ Secret Manager lookup bypassed: {e}")

    #     # 2. Fall back to local Environment Variable
    #     fallback_token = os.getenv("GITHUB_TOKEN")
    #     if fallback_token:
    #         print(" > Using fallback GITHUB_TOKEN from local environment configuration.")
    #         return fallback_token.strip()
            
    #     raise RuntimeError("Could not resolve a GitHub Token from either GCP Secret Manager or local environment variables.")
    ...
    def _collect_files(self):
        ...
        if not collected_paths and self.target_directory and os.path.exists(self.target_directory):
            ...
            # Add security filters matching your .gitignore rules
            ignore_dirs = {'.git', '__pycache__', 'node_modules', '.venv', 'venv'}
            ignore_files = {'.DS_Store', '.env', 'agent.py'}
        ...
        while self.state not in [PRState.SUCCESS, PRState.ERROR]:
            if self.state == PRState.INIT:
                ...
                try:
                    ...
                    # Automatically launch the web browser to the opened PR URL
                    try:
                        print(f" > Launching web browser to: {self.pr.html_url}")
                        webbrowser.open(self.pr.html_url, new=2) # new=2 opens it in a new tab if possible
